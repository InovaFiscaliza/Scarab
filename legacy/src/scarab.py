...
def main(config_path: str) -> None:
    ...
    while keep_watching:
        try:
            metadata_to_process, data_files_to_process, metadata_found, data_found = (
                fh.get_files_to_process()
            )
            ...
            if data_found:
                ...
                for file_type, data_file_set in data_files_to_process.items():
                    data_file_set.difference_update(
                        dh.data_files_to_ignore.get(file_type, set())
                    )
            ...
            log.error(
                config.exception_message_handling(f"Unhandled error in main loop: {e}")
            )
