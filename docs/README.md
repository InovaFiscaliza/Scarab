...
| metadata | **in columns** | `List of Dict`. List of columns required for posted metadata for each table. Will be used to validate the new files. May not include all existing columns, but only the essential ones. Keys are automatically required and may not be included in the list. | Optional | default: [] |
| metadata | **key** | `List of Dict`. List of columns to be used as key in the metadata for each table. This is used for merging data. Each row will have a single combination of the key columns. If new data is posted with the same key column combination, the data will be updated | Optional | default:[] |
| metadata | **association** | `List of Strings`. List containing dictionaries for table association through the use of "Primary Keys" (PK) and "Foreign Keys" (FK) when processing multi-table data. e.g. [{"DimData1": {"PK": "PK1"}},{"FactData": {"FK": [{"DimData1": "FK1"}]}}], where `DimData1` and `FactData` are table names defined in the json data file. The value associated with the `PK` key corresponds to the column name associated for that table to be used in cross-references, and the list associated with the `FK` key indicates the name of columns with data association with other tables through their PK values. A table may have none or a single PK column, and may include none or multiple columns with references to other tables. The PK and FL values are valid only within a single file. To use multiple files, an external indexing system must be used and Scarab may be used with multiple workflows, one for each table.  | Optional | default: [] |
