...
| metadata | **add filename** | `Dict`. Specify how the filename of the metadata file will be added to the output, regardless of being present as a datafield in input files. Defined in the form of a dictionary using the structure: `{"<table name>":"<column name>"}` containing the name of the column to be created to hold the filename within each table. If left empty, will not create any column. | Optional | default: {} |
| metadata | **filename format** | `String`. Format string used to parse information from the filename. Based in the re.match.groupdict method. See [re](https://docs.python.org/3/library/re.html#re.Match.groupdict) for more information. | Optional | default: "" |
| | | | | |
