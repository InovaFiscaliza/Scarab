...
| files | **worksheet names** | `List`. String with the worksheet name for a single worksheet or a list of dictionaries containing the worksheet names for multiple tables. If not included, worksheet will be created with the configuration name with added serial number if multiple worksheets are created. e.g [{"json_root_table_name": "worksheet_name"}, ...]. May use the key "_" as reference to data in the first level of the json tree with keys not defined in the other elements of the list, i.e. works as a default to include all data within the first level.  | Optional | default: [{"_":"<string defined in 'name' configuration key>"}] |
